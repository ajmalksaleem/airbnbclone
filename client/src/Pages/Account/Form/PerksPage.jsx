...
function Perks({ selected, onChange }) {
    ...
    function handleCbClick(ev) {
        ...
        if (checked) {
            ...
            onChange(...selected.filter(selectedName => selectedName !== name))  //if we uncheck the same this will work ,For each item in the selected array, keep it only if its name is different from the name of the checkbox that was just clicked."  or hear voice 'perks' on telegram
        }
    }

    return (
        <div className='grid grid-cols-2 md:grid-cols-3  lg:grid-cols-6 gap-1 mt-3 bg-white'>
            ...
            <label className='border p-4 flex rounded-2xl gap-2 items-center cursor-pointer'>
                <input checked={selected.includes('freeparking')} name='freeparking' onChange={handleCbClick} type="checkbox" />
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
            ...
            <label className='border p-4 flex rounded-2xl gap-2 items-center cursor-pointer'>
                <input checked={selected.includes('privateentrance')} name='privateentrance' onChange={handleCbClick} type="checkbox" />
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
